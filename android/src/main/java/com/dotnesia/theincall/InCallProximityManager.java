...
package com.dotnesia.theincall;
...
import com.dotnesia.theincall.AppRTC.AppRTCProximitySensor;
...
public class InCallProximityManager {
    ...
    /** Construction */
    static InCallProximityManager create(Context context, final TheincallPlugin inCallManager) {
        return new InCallProximityManager(context, inCallManager);
    }

    private InCallProximityManager(Context context, final TheincallPlugin inCallManager) {
        Log.d(TAG, "Theincall.InCallProximityManager");
        ...
        if (proximitySupported) {
            proximitySensor = AppRTCProximitySensor.create(context,
                new Runnable() {
                    @Override
                    public void run() {
                        Log.d(TAG,"Theincall.InCallProximityManager:AppRTCProximitySensor:run()");
    ...
    private void checkProximitySupport(Context context) {
        ...
        try {
            ...
            Log.d(TAG, "Theincall.Failed to get proximity screen locker. exception: ", e);
        }

        if (mProximityLock != null) {
            Log.d(TAG, "Theincall.use native screen locker...");
            try {
                mPowerManagerRelease = mProximityLock.getClass().getDeclaredMethod("release", int.class);
            } catch (Exception e) {
                Log.d(TAG, "Theincall.failed to get proximity screen locker: `release()`. exception: ", e);
            }
        } else {
            Log.d(TAG, "Theincall.fallback to old school screen locker...");
        }
    }
    ...
    public void acquireProximityWakeLock() {
        ...
        synchronized (mProximityLock) {
            if (!mProximityLock.isHeld()) {
                Log.d(TAG, "Theincall.acquireProximityWakeLock()");
        ...
        synchronized (mProximityLock) {
            if (mProximityLock.isHeld()) {
                try {
                    ...
                    Log.d(TAG, "Theincall.releaseProximityWakeLock()");
                } catch (Exception e) {
                    Log.e(TAG, "Theincall.failed to release proximity lock. e: ", e);
                }
            }
        }
    }
