...
import com.dotnesia.theincall.AppRTC.AppRTCBluetoothManager;
...
public class TheincallPlugin implements FlutterPlugin, MethodCallHandler {
  ...
  private String mPackageName = "com.dotnesia.theincall";
            ...
            if (tg != null) {
                synchronized (this) {
                    if (!playing) {
                        ...
                        TheincallPlugin.this.updateAudioRoute();
...
// ===== Internal Classes End =====
    ...
    public void checkRecordPermission(Result result) {
        Log.d(TAG, "Theincall.checkRecordPermission(): enter");
        _checkRecordPermission();
        if (recordPermission.equals("unknow")) {
            Log.d(TAG, "Theincall.checkRecordPermission(): failed");
    ...
    public void checkCameraPermission(Result result) {
        Log.d(TAG, "Theincall.checkCameraPermission(): enter");
        _checkCameraPermission();
        if (cameraPermission.equals("unknow")) {
            Log.d(TAG, "Theincall.checkCameraPermission(): failed");
    ...
    private void _checkRecordPermission() {
        ...
        Log.d(TAG, String.format("Theincall.checkRecordPermission(): recordPermission=%s", recordPermission));
    }

    private void _checkCameraPermission() {
        cameraPermission = _checkPermission(permission.CAMERA);
        Log.d(TAG, String.format("Theincall.checkCameraPermission(): cameraPermission=%s", cameraPermission));
    }
    ...
    public void requestRecordPermission(Result result) {
        Log.d(TAG, "Theincall.requestRecordPermission(): enter");
        _checkRecordPermission();
    ...
    public void requestCameraPermission(Result result) {
        Log.d(TAG, "Theincall.requestCameraPermission(): enter");
        _checkCameraPermission();
